...
#ifndef SERVEREXCLUSIVEDEVIATIONALARM_H
#define SERVEREXCLUSIVEDEVIATIONALARM_H
#if TB5_ALARMS
#include "Base.h"
#include "Enums.h"
...
namespace SoftingOPCToolbox5
{
	...
	namespace Server
	{
		...
		*
		* The ExclusiveDeviationAlarm is utilized with multiple mutually exclusive limits. */
		class TBC_EXPORT ExclusiveDeviationAlarm
			: public ExclusiveLimitAlarm
		{
		public:
			/*! Constructs a new ExclusiveDeviationAlarm instance */
			static ObjectPointer<ExclusiveDeviationAlarm> create();

			/*! Constructor that shall be used by derived classes whose instances are created by the Creator object factory */
			ExclusiveDeviationAlarm(TBHandle otbHandle);

		protected:
			/*! Default constructor */
			ExclusiveDeviationAlarm();
			/*! For internal use only */
			ExclusiveDeviationAlarm(bool suppressCoreInstanceCreate);

		public:
			/*! Destructor */
			virtual ~ExclusiveDeviationAlarm();
		...
		private:
			...
			ExclusiveDeviationAlarm& operator=(const ExclusiveDeviationAlarm&);
			/*! Forbid use of copy constructor */
			ExclusiveDeviationAlarm(const ExclusiveDeviationAlarm&);
		};
		typedef ObjectPointer<ExclusiveDeviationAlarm> ExclusiveDeviationAlarmPtr;
		typedef ObjectPointer<const ExclusiveDeviationAlarm> ExclusiveDeviationAlarmConstPtr;

	} // end Server namespace
} // toolbox namespace end

#endif
#endif	// SERVEREXCLUSIVEDEVIATIONALARM_H
